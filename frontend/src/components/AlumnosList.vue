<template>
  <div class="alumnos-container">
    ...
    <div class="alumnos-content">
      <div v-if="alumnosFiltrados.length > 0" class="alumnos-grid">
        <div v-for="alumno in alumnosFiltrados" :key="alumno.id" class="alumno-card">
          ...
          <div class="alumno-info">
            <h4 class="alumno-nombre">{{ alumno.nombre }} {{ alumno.apellido }}</h4>
            <p class="alumno-id">Correo: {{ alumno.email }}</p>
            <p class="alumno-id">Teléfono: {{ alumno.telefono }}</p>
          </div>
          <div class="alumno-acciones">
            <button class="btn btn-icon btn-ver">
              <i class="bi bi-eye-fill"></i>
            </button>
            <button class="btn btn-icon btn-editar" @click="abrirModalEditar(alumno)">
              <i class="bi bi-pencil-fill"></i>
            </button>
            <button class="btn btn-icon btn-eliminar" @click="abrirModalEliminar(alumno)">
      ...
      <div v-else class="estado-vacio">
        ...
        <h3 class="vacio-titulo">No hay alumnos registrados</h3>
        <p class="vacio-descripcion">
          Cuando se registren alumnos, aparecerán en esta lista.
        </p>
    ...
    <div class="modal-container" v-if="modalAgregar">
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <form @submit.prevent="guardarNuevoAlumno">
            <div class="row g-3">
              <div class="col-md-6">
              ...
              <div class="col-md-6">
                <div class="form-floating">
                  <input 
                    ...
                    v-model="nuevoAlumno.fecha_nacimiento"
                    required
                  >
    ...
    <div class="modal-container" v-if="modalEditar">
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <form @submit.prevent="guardarEdicionAlumno">
            <div class="row g-3">
              <div class="col-md-6">
                <div class="form-floating">
                  <input 
              ...
              <div class="col-md-6">
                <div class="form-floating">
                  <input 
                    ...
                    id="edit-apellido" 
                    placeholder="Apellido"
                    v-model="alumnoEditando.apellido"
                    required
                  >
              ...
              <div class="col-md-6">
                <div class="form-floating">
                  <input 
                    ...
                    placeholder="Fecha de Nacimiento"
                    v-model="alumnoEditando.fecha_nacimiento"
                    required
                  >
              ...
              <div class="col-md-6">
                <div class="form-floating">
                  <input 
                    ...
                    id="edit-telefono" 
                    placeholder="Teléfono"
                    v-model="alumnoEditando.telefono"
                  >
                ...
                <div class="form-floating">
                  <div class="form-floating">
                    <select 
                      class="form-select" 
                      id="edit-grado" 
                      v-model="alumnoEditando.id_grupo"
                      required
                    >
                      <option value="" disabled selected>Selecciona un grupo</option>
                      <option v-for="grupo in grupos" :key="grupo.id_grupo" :value="grupo.id_grupo">
                    ...
                    <label for="edit-grado">Grupo</label>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div class="modal-container" v-if="modalEliminar">
      <div class="modal-content modal-sm" @click.stop>
        <div class="modal-header">
          <h3 class="modal-title">
            <i class="bi bi-exclamation-triangle-fill me-2 text-danger"></i>Eliminar Alumno
          </h3>
        ...
        <div class="modal-body">
          <div class="text-center mb-4">
            <div class="avatar-eliminar">
              {{ alumnoEliminando ? obtenerIniciales(alumnoEliminando.nombre, alumnoEliminando.apellido) : '' }}
            </div>
            <h4 class="mt-3">{{ alumnoEliminando ? `${alumnoEliminando.nombre} ${alumnoEliminando.apellido}` : '' }}</h4>
          </div>
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-eliminar" @click="confirmarEliminarAlumno">
...
// Importa las funciones de la API para alumnos
import { getAlumnos, createAlumno, updateAlumno, deleteAlumno, getGrupos, generarReporteAlumnos } from '../services/api'
...
// ===============================
// Estado reactivo para la lista de alumnos y búsqueda
// ===============================
const alumnos = ref<Alumno[]>([])
const busqueda = ref('')

// ===============================
// Estado para los modales y alumno en edición/eliminación
// ===============================
const modalAgregar = ref(false)
const modalEditar = ref(false)
const modalEliminar = ref(false)
const alumnoEditando = ref<Alumno | null>(null)
const alumnoEliminando = ref<Alumno | null>(null)

// ===============================
// Estado para el nuevo alumno (sin id)
// ===============================
const nuevoAlumno = ref<Omit<Alumno, 'id'>>({
  nombre: '',
  apellido: '',
  email: '',
  telefono: '',
...
// ===============================
// Computed para filtrar alumnos según la búsqueda
// ===============================
...
// ===============================
// Obtiene iniciales para el avatar del alumno
// ===============================
...
// ===============================
// Modal: Abrir y cerrar para agregar alumno
// ===============================
function abrirModalAgregar() {
  // Reinicia el formulario de nuevo alumno
  nuevoAlumno.value = {
    nombre: '',
    apellido: '',
    email: '',
...
// ===============================
// Guardar un nuevo alumno usando la API
// ===============================
async function guardarNuevoAlumno() {
  try {
    const res = await createAlumno(nuevoAlumno.value)
    if (res.mensaje) {
      mostrarAlerta(res.mensaje, 'exito')
      await cargarAlumnos() // Recarga la lista tras agregar
      cerrarModalAgregar()
    } else {
      mostrarAlerta(res.error || 'Error al agregar alumno', 'error')
    }
  } catch (error) {
    mostrarAlerta('Error al agregar alumno', 'error')
  }
}

// ===============================
// Modal: Abrir y cerrar para editar alumno
// ===============================
function abrirModalEditar(alumno: Alumno) {
  alumnoEditando.value = { ...alumno } // Clona el alumno
...
// ===============================
// Guardar los cambios de un alumno usando la API
// ===============================
async function guardarEdicionAlumno() {
  if (alumnoEditando.value) {
    try {
      const res = await updateAlumno(alumnoEditando.value.id, alumnoEditando.value)
      if (res.mensaje) {
        mostrarAlerta(res.mensaje, 'exito')
        await cargarAlumnos() // Recarga la lista tras editar
        cerrarModalEditar()
      } else {
        mostrarAlerta(res.error || 'Error al actualizar alumno', 'error')
      }
    } catch (error) {
      mostrarAlerta('Error al actualizar alumno', 'error')
    }
  }
}

// ===============================
// Modal: Abrir y cerrar para eliminar alumno
// ===============================
function abrirModalEliminar(alumno: Alumno) {
  alumnoEliminando.value = alumno
  modalEliminar.value = true
}

function cerrarModalEliminar() {
  modalEliminar.value = false
  alumnoEliminando.value = null
}

// ===============================
// Confirmar y eliminar un alumno usando la API
// ===============================
async function confirmarEliminarAlumno() {
  if (alumnoEliminando.value) {
    try {
      const res = await deleteAlumno(alumnoEliminando.value.id)
      if (res.mensaje) {
        mostrarAlerta(res.mensaje, 'exito')
        await cargarAlumnos() // Recarga la lista tras eliminar
        cerrarModalEliminar()
      } else {
        mostrarAlerta(res.error || 'Error al eliminar alumno', 'error')
      }
    } catch (error) {
      mostrarAlerta('Error al eliminar alumno', 'error')
    }
  }
}
...
async function generarReporte() {
  try {
    const response = await fetch('http://localhost/alumnos/reporte/pdf', {
      credentials: 'include'
    })
